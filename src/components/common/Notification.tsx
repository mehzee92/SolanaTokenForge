...
const Notification = ({ type, message, description, txid, onHide }) => {
  ...
  return (
    <div className={`max-w-lg w-full bg-white shadow-lg rounded-md mt-2 pointer-events-auto ring-1 ring-black ring-opacity-5 p-2 mx-auto mt-16 overflow-hidden opacity-70`}>
      <div className={`p-4`}>
        <div className={`flex items-center text-center`}>
          <div className={`flex-shrink-0`}>
            {type === 'success' ? (
              <Image src="/ok.png" alt="Success" width={32} height={32} layout="fixed" />
            ) : null}
            {type === 'info' && <InformationCircleIcon className={`h-8 w-8 mr-1 text-blue-700`} />}
            {type === 'error' && (
              <Image src="/bug.png" alt="error" width={32} height={32} layout="fixed" />
            )}
          </div>
